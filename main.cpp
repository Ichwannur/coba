...
#include <GL/glut.h> //library grafik MASIHCOBA
    ...
    void blok(float tebal,int ratiol,int ratiop); //menggambar blok MASIH COBA KE 3
    ...
    double rotation_y=0,rotation_y_plus=0,direction;// mengatur rotasi agar searah jarum jam, y=-10 utk
    ...
    bool speed1=true,speed2=false,speed3=false,speed4=false,f5=true; //kecepatan awal kipasdi speed1
    ...
    glColorMaterial (GL_FRONT,GL_AMBIENT_AND_DIFFUSE);//Nilai yang diterima adalahmasi m
    glClearColor(0.75,0.75,0.75,0.75); //backgorund
    }
...
//E1E121004 LA ODE LUKMANA
    // definisikan kondisi pergerakan penolehan
     ...
     cylinder(0.0,0.0,0.0);// silinder belakang kipas atau BAGIAN KEPALA KIPAS YANG TERDAPAT TOBOL NAIKNYA
     ...
     cylinder(0.3,0.5,6);
     else // pull up turn left-right head button -- SAMAPAI DISINI
     cylinder(0.3,0.5,7);
     glRotatef(90,1.0,0.0,0.0);
         glPushMatrix();
...
void bilah (float r_inner,float r_outer,float tebal,int batang)
    ...
    case GLUT_KEY_F2://speed ke-2
        if(speed2 == false)//pengkondisian awal, jika speed2 false maka kerjakan
        {
...
// interaksi melalui mouse
 ...
 int main(int argc,char **argv)
    {
        glutInit(&argc,argv);
        //glClearColor3i(1,1,1);
        glutInitDisplayMode(GLUT_DOUBLE|GLUT_RGB|GLUT_DEPTH);
        glutInitWindowSize(screen_width,screen_height);
        glutInitWindowPosition(0,0);
        glutCreateWindow("KELOMPOK 4 OCINO");
